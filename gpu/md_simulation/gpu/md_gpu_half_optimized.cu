...
__global__ void PairCorrelationFunctionGPU(int n, int nbins, double box, double binsize, double * x, double * y, double * z, unsigned int * g,int * neighbors,int * n_neighbors, int maxneighbors);
__global__ void NeighborsGPU(int n,double box, double * x,double * y,double * z,int * neighbors, int * n_neighbors,int maxneighbors, double r2cut);
...
int main (int argc, char* argv[]) {
    ...
    int maxneighbors = 0;
    for (int i = 0; i < n; i++) {
        if ( maxneighbors < neighbors[i].size()) maxneighbors = neighbors[i].size();
    }
    maxneighbors *= 1.5;
    ...
    cudaMemset((void*)gpu_n_neighbors,'\0',n*sizeof(unsigned int));
    ...
    NeighborsGPU<<<dimgrid,threads_per_block>>>(n,box,gpu_x,gpu_y,gpu_z,gpu_neighbors,gpu_n_neighbors,maxneighbors,r2cut);
    ...
    do { 
        ...
        UpdateAccelerationGPU(n,box,x,y,z,ax,ay,az,gpu_x,gpu_y,gpu_z,gpu_ax,gpu_ay,gpu_az,gpu_neighbors,gpu_n_neighbors,maxneighbors);
        end = omp_get_wtime();
        acc_time += end - start;
        ...
        if ( iter > 199 && iter % 100 == 0 ) { 
            start = omp_get_wtime();
            PairCorrelationFunctionGPU<<<dimgrid,threads_per_block>>>(n,nbins,box,binsize,gpu_x,gpu_y,gpu_z,gpu_g,gpu_neighbors,gpu_n_neighbors,maxneighbors);
            cudaThreadSynchronize();
            npts++;
            end = omp_get_wtime();
            cor_time += end - start;
